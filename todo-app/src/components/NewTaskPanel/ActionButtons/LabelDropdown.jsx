...
const LabelDropdown = forwardRef(({ onSelect, lightMode }, ref) => {
      ...
      {open && (
        ...
        >
          {LABELS.map((label) => (
            ...
            >
              {label.icon ? <span>{label.icon}</span> : <span className={`w-3 h-3 rounded-full ${label.color}`} />}
